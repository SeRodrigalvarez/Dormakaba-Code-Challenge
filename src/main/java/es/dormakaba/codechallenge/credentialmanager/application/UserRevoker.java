...
import es.dormakaba.codechallenge.iotsimulator.application.DoorCredentialsRemover;
import es.dormakaba.codechallenge.iotsimulator.domain.exception.DoorNotExistException;

@Service
public class UserRevoker {

    @Autowired
    private DoorCredentialsRemover doorCredentialsRemover;
    ...
    private static final Logger logger = LoggerFactory.getLogger(UserRevoker.class);

    public void revoke (UUID userId, UUID doorId) throws UserNotExistException, DoorNotExistException {
        User user = this.userRepository.getById(userId);
        this.doorCredentialsRemover.doorCredentialsRemover(doorId, user.getCredentialIds());
        ...
        for (UUID credentialId : user.getCredentialIds()) {
            try {
                ...
                credential.removeDoorId(doorId);
                this.credentialRepository.update(credential);
            } catch (CredentialNotExistException e) {
                logger.error("Unexpected CredentialNotExistException at UserRevoker use case");
            }
        }
    }
}
